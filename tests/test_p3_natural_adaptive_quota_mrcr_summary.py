...
from run_p3_mrcr import ADAPTIVE_QUOTA_ARMS  # noqa: E402
from summarize_p3_natural_adaptive_quota_mrcr import (  # noqa: E402
...
def _record(*, arm: str, score: float, adaptive: bool) -> dict[str, object]:
    return {
        "example_id": "2n:0:example",
        "raw_prompt_sha256": "a" * 64,
        "input_token_ids_sha256": "b" * 64,
        "exact_input_tokens": 100,
        "generation_reserve_tokens": 8,
        "needle_count": 2,
        "official_bin_index": 0,
        "official_o200k_prompt_plus_answer_tokens": 100,
        "status": "scored",
        "score": score,
        "failure_type": None,
...
def _manifest() -> dict[str, object]:
    return {
        ...
        "confirmation_gate": {
            ...
            "nonnegative_needle_bin_count_minimum": 1,
            "worst_needle_bin_regression_minimum": -0.08,
...
def test_mrcr_bootstrap_is_deterministic() -> None:
...
def test_mrcr_analysis_applies_cell_holm_and_physical_gate() -> None:
    fixed = [_record(arm=ADAPTIVE_QUOTA_ARMS[0], score=0.0, adaptive=False)]
    adaptive = [_record(arm=ADAPTIVE_QUOTA_ARMS[1], score=1.0, adaptive=True)]
    ...
    assert result["multiplicity"]["family_size"] == 1
    assert result["initial_prefill_physical"]["paired_successful_quota_examples"] == 1


def test_mrcr_analysis_rejects_cross_arm_global_budget_drift() -> None:
    fixed = [_record(arm=ADAPTIVE_QUOTA_ARMS[0], score=0.0, adaptive=False)]
    adaptive = [_record(arm=ADAPTIVE_QUOTA_ARMS[1], score=0.0, adaptive=True)]
...
def test_mrcr_rejects_unregistered_failure() -> None:
    record = {
        "example_id": "2n:0:example",
        "status": "failure",
    ...
    with pytest.raises(ValueError, match="Unregistered adaptive MRCR failure"):
        verify_quota_records([record], arm=ADAPTIVE_QUOTA_ARMS[0], allowed_failures={"oom"})


def test_mrcr_all_failure_grid_cannot_pass_confirmation() -> None:
    fixed = [_record(arm=ADAPTIVE_QUOTA_ARMS[0], score=0.0, adaptive=False)]
    adaptive = [_record(arm=ADAPTIVE_QUOTA_ARMS[1], score=0.0, adaptive=True)]
    ...
    assert result["confirmation_gate"]["checks"][
        "at_least_one_jointly_successful_prefill"
    ] is False


def test_mrcr_summary_identity_is_distinct() -> None:
    assert SUMMARY_EXPERIMENT_ID == "p3-natural-adaptive-quota-mrcr-audit-v1"
