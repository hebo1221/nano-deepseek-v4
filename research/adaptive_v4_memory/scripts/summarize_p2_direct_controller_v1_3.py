...
if __name__ == "__main__":
    ...
    _expected_entrypoint = (
        str(_source_provenance["repository_root"]).rstrip("/") + "/research/adaptive_v4_memory/scripts/summarize_p2_direct_controller_v1_3.py"
    )
    if (
        _launch_routing.get("schema_version") != 1
        or _launch_routing.get("launcher")
        != "p2-direct-controller-git-object-launcher-v1-3-5"
        or _launch_routing.get("entrypoint_selector") != 'summary'
        or _launch_routing.get("entrypoint_relative_path") != 'research/adaptive_v4_memory/scripts/summarize_p2_direct_controller_v1_3.py'
...
if globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_5") is not None:
    for authority_target in (integrity_audit, integrity_audit.matrix):
        ...
        typed_authority_target.SEALED_LAUNCH_AUTHORITY_V1_3_5 = dict(
            cast(Mapping[str, Any], SEALED_LAUNCH_AUTHORITY_V1_3_5)
        )
        typed_authority_target.SEALED_PYTHON_RUNTIME_V1_3_5 = dict(
            cast(Mapping[str, Any], SEALED_PYTHON_RUNTIME_V1_3_5)
        )
        typed_authority_target.SEALED_SOURCE_PROVENANCE_V1_3_5 = dict(
            cast(Mapping[str, Any], SEALED_SOURCE_PROVENANCE_V1_3_5)
        )

EXPERIMENT_ID = contract.V1_3_5_SUMMARY_EXPERIMENT_ID
ARTIFACT_TYPE = "paper-grade-direct-controller-exact-fill-summary"
SCHEMA_VERSION = 1
ATTESTATION_PURPOSE = contract.V1_3_5_SUMMARY_ATTESTATION_PURPOSE
REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_OUTPUT = contract.V1_3_5_SUMMARY_OUTPUT_PATH
...
def _active_summary_launch_routing() -> dict[str, Any]:
    raw = globals().get("SEALED_LAUNCH_ROUTING_V1_3_5")
    raw_authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_5")
    raw_runtime = globals().get("SEALED_PYTHON_RUNTIME_V1_3_5")
    raw_source = globals().get("SEALED_SOURCE_PROVENANCE_V1_3_5")
    _require(
...
def _validate_summary_launch_routing_snapshot(
    ...
    *,
    ...
    if globals().get("SEALED_LAUNCH_ROUTING_V1_3_5") is not None:
        _require(
...
def _revalidate_summary_source_bindings(
    ...
    *,
    ...
    replay = validate_frozen_analysis_provenance(
        implementation_tree_sha256=contract.v1_3_5_implementation_tree_digest(),
...
def _validate_system_semantics(
    ...
    *,
    ...
    _require(
        len(slices) == len(contract.ALL_ARM_NAMES) * len(expected_cells) == 15_300
    ...
    _require(
        len(per_arm) == len(contract.ALL_ARM_NAMES) == 17
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--matrix-summary",
        type=Path,
        default=contract.V1_3_5_MATRIX_SUMMARY_PATH,
    )
    parser.add_argument(
        "--raw-output-root",
        type=Path,
        default=contract.V1_3_5_OUTPUT_ROOT,
    )
...
def main() -> None:
    ...
    final_implementation_tree_sha256 = contract.v1_3_5_implementation_tree_digest()
