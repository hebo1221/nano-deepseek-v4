...
if globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_4") is not None:
    for authority_target in (integrity_audit, integrity_audit.matrix):
        ...
        typed_authority_target.SEALED_LAUNCH_AUTHORITY_V1_3_4 = dict(
            cast(Mapping[str, Any], SEALED_LAUNCH_AUTHORITY_V1_3_4)
        )
        typed_authority_target.SEALED_PYTHON_RUNTIME_V1_3_4 = dict(
            cast(Mapping[str, Any], SEALED_PYTHON_RUNTIME_V1_3_4)
        )
        typed_authority_target.SEALED_SOURCE_PROVENANCE_V1_3_4 = dict(
            cast(Mapping[str, Any], SEALED_SOURCE_PROVENANCE_V1_3_4)
        )

EXPERIMENT_ID = contract.V1_3_4_SUMMARY_EXPERIMENT_ID
ARTIFACT_TYPE = "paper-grade-direct-controller-exact-fill-summary"
SCHEMA_VERSION = 1
ATTESTATION_PURPOSE = contract.V1_3_4_SUMMARY_ATTESTATION_PURPOSE
REPOSITORY_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_OUTPUT = contract.V1_3_4_SUMMARY_OUTPUT_PATH
...
def _active_summary_launch_routing() -> dict[str, Any]:
    raw = globals().get("SEALED_LAUNCH_ROUTING_V1_3_4")
    raw_authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_4")
    raw_runtime = globals().get("SEALED_PYTHON_RUNTIME_V1_3_4")
    raw_source = globals().get("SEALED_SOURCE_PROVENANCE_V1_3_4")
    _require(
...
def _validate_summary_launch_routing_snapshot(
    ...
    *,
    ...
    if globals().get("SEALED_LAUNCH_ROUTING_V1_3_4") is not None:
        _require(
...
def _revalidate_summary_source_bindings(
    ...
    *,
    ...
    replay = validate_frozen_analysis_provenance(
        implementation_tree_sha256=contract.v1_3_4_implementation_tree_digest(),
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        "--matrix-summary",
        type=Path,
        default=contract.V1_3_4_MATRIX_SUMMARY_PATH,
    )
    parser.add_argument(
        "--raw-output-root",
        type=Path,
        default=contract.V1_3_4_OUTPUT_ROOT,
    )
...
def main() -> None:
    ...
    final_implementation_tree_sha256 = contract.v1_3_4_implementation_tree_digest()
