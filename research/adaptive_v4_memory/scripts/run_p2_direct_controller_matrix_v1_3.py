...
globals().setdefault("SEALED_PYTHON_RUNTIME_V1_3_3", None)
globals().setdefault("SEALED_LAUNCH_AUTHORITY_V1_3_3", None)
globals().setdefault("SEALED_SOURCE_PROVENANCE_V1_3_3", None)
globals().setdefault("SEALED_LAUNCH_ROUTING_V1_3_3", None)

DEFAULT_GPU_LOCK_PATH = contract.DIRECT_GPU_SCHEDULER_LOCK_PATH

EXPERIMENT_ID = contract.V1_3_3_MATRIX_EXPERIMENT_ID
WORKER_EXPERIMENT_ID = contract.V1_3_3_WORKER_LEDGER_EXPERIMENT_ID
ARTIFACT_TYPE = "direct-controller-exact-fill-quality-matrix"
SCHEMA_VERSION = 1
MATRIX_ATTESTATION_PURPOSE = contract.V1_3_3_MATRIX_ATTESTATION_PURPOSE
WORKER_ATTESTATION_PURPOSE = contract.V1_3_3_WORKER_LEDGER_ATTESTATION_PURPOSE
...
EVALUATOR_FD_ENV = "ADAPTIVE_V4_CANONICAL_DIRECT_EXACT_FILL_V1_3_3_EVALUATOR_FD"
IMPORT_INVENTORY_FD_ENV = "ADAPTIVE_V4_DIRECT_EXACT_FILL_V1_3_3_IMPORT_INVENTORY_FD"
...
def _worker_ledger_root(output_root: Path) -> Path:
    ...
    canonical_output = _absolute(contract.V1_3_3_OUTPUT_ROOT)
    if root == canonical_output:
        canonical_worker_root = _absolute(contract.V1_3_3_WORKER_LEDGER_ROOT)
        _require(
            canonical_worker_root.parent == root.parent,
            "Canonical v1.3.3 worker-ledger root drifted.",
        )
        return canonical_worker_root
    suffix = contract.V1_3_3_WORKER_LEDGER_ROOT.name.removeprefix(
        f".{contract.V1_3_3_OUTPUT_ROOT.name}."
    )
    _require(bool(suffix), "V1.3.3 worker-ledger suffix is empty.")
...
class FrozenPrerequisites:
    context: admission.QualityContext
    trust_root: attestation.TrustRoot
    activation: admission.ValidatedQualityStartActivationV1_3_3
...
def _matrix_lock_path(output_root: Path) -> Path:
    root = _absolute(output_root)
    _require(
        root == _absolute(contract.V1_3_3_OUTPUT_ROOT),
        "V1.3.3 matrix lock is defined only for the canonical quality output root.",
    )
    return _absolute(contract.V1_3_3_ACTIVATION_MATRIX_LOCK_PATH)
...
def _validate_matrix_layout(
    *,
    ...
    root = _exact_resolved_path(output_root, label="Controller v1.3.3 output root")
    summary = _exact_resolved_path(matrix_summary, label="Controller v1.3.3 matrix summary")
    _require(
        root == _exact_resolved_path(
            contract.V1_3_3_OUTPUT_ROOT, label="Canonical controller v1.3.3 output root"
        )
        and summary == root / MATRIX_SUMMARY_NAME,
        "Controller v1.3.3 output and matrix summary must be canonical.",
    )
    ...
    _require(
        ...
        == _exact_resolved_path(
            contract.V1_3_3_REUSE_ADMISSION_PATH,
        ...
        "Controller v1.3.3 admission/genesis paths must be canonical.",
    )
    activation_root = _exact_resolved_path(
        contract.V1_3_3_ACTIVATION_ROOT, label="Quality-start activation root"
    )
    activation_path = _exact_resolved_path(
        contract.V1_3_3_QUALITY_START_ACTIVATION_PATH,
        label="Quality-start activation receipt",
    )
    lock_path = _exact_resolved_path(
        contract.V1_3_3_ACTIVATION_MATRIX_LOCK_PATH,
    ...
    if attestation_key_path is not None:
        ...
        _require(
            ...
            and not _paths_overlap(
                key,
                _exact_resolved_path(
                    admission.V1_3_3_ACTIVATION_BOOTSTRAP_LOCK_PATH,
...
def _opened_matrix_lock_binding(
    ...
    _require(
        ...
        "V1.3.3 matrix locks can only be adopted from typed activation authority.",
    )
    no_follow = getattr(os, "O_NOFOLLOW", None)
...
def _initialize_matrix_lock_binding(path: Path) -> dict[str, Any]:
    del path
    raise RuntimeError(
        "V1.3.3 matrix lock creation is private to publish_quality_start_activation."
    )
...
def _establish_prerequisite_context(
    *,
    ...
    roots = (
        ...
        output_root,
        _absolute(contract.V1_3_3_ADMISSION_ROOT),
        _absolute(contract.V1_3_3_ACTIVATION_ROOT),
    )
...
def _build_base_prerequisites(
    *,
    ...
    _require(
        ...
        "Controller v1.3.3 quality execution is frozen to one worker.",
    )
    _require(
...
def load_and_validate_prestart_prerequisites(
    *,
    ...
    attestation_key_path: Path | None,
    worker_count: int,
) -> tuple[ValidatedBasePrerequisites, admission.PrestartQualityAuthorityV1_3_3]:
...
def load_and_validate_prerequisites(
    *,
    ...
    if route is None:
        raw_route = globals().get("SEALED_LAUNCH_ROUTING_V1_3_3")
...
def _quality_lock_protected_paths(
    *,
    ...
    paths = [
        _exact_resolved_path(manifest_path, label="Controller v1.3.3 manifest"),
        layout.output_root,
        ...
        _exact_resolved_path(
            contract.V1_3_3_INTEGRITY_OUTPUT_PATH,
            label="Controller integrity artifact",
        ),
        _exact_resolved_path(
            contract.V1_3_3_SUMMARY_OUTPUT_PATH,
    ...
    paths.extend(
        ...
        for relative in contract.V1_3_3_IMPLEMENTATION_PATHS
    )
    if attestation_key_path is not None:
        paths.append(
    ...
    *,
    ...
    if require_canonical_scheduler:
        _require(
            ...
            "Controller v1.3.3 requires the canonical GPU scheduler lock path.",
        )
    _require(
...
def _prepare_quality_start_authority(
    *,
    ...
    lease: admission.QualityStartActivationLeaseV1_3_3 | None = None
    returned = False
    try:
        if start_mode == "fresh":
            _require(
            ...
            if fresh_prestart is None:
                ...
                _require(
                    type(base) is ValidatedBasePrerequisites
                    and type(prestart) is admission.PrestartQualityAuthorityV1_3_3,
...
def _activation_lock_binding(
    ...
    lease: admission.QualityStartActivationLeaseV1_3_3,
) -> dict[str, Any]:
    lease.assert_held()
    _require(
...
def _validated_source_provenance() -> dict[str, Any]:
    value = globals().get("SEALED_SOURCE_PROVENANCE_V1_3_3")
    _require(
...
def _require_launcher_authority() -> dict[str, Any]:
    authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_3")
    _require(
        authority
        == {
            "schema_version": 1,
...
def _validated_launch_routing(*, expected_selector: str) -> dict[str, Any]:
    value = globals().get("SEALED_LAUNCH_ROUTING_V1_3_3")
    expected_paths = {
        "matrix": (
...
def _validated_launcher_python_runtime() -> dict[str, Any]:
    runtime = globals().get("SEALED_PYTHON_RUNTIME_V1_3_3")
    _require(
...
def _sealed_import_inventory_fd(context: admission.QualityContext) -> int:
    ...
    descriptor = cast(Callable[[str, int], int], create)(
        "adaptive-v4-direct-exact-fill-v1-3-3-import-inventory",
        cast(int, getattr(os, "MFD_CLOEXEC", 0)) | cast(int, allow_sealing),
    )
    try:
        offset = 0
...
def _isolated_evaluator_pycache_prefix(launch_nonce: str) -> Path:
    ...
    path = root / f"adaptive-v4-direct-exact-fill-v1-3-3-pycache-{launch_nonce}"
    _require(
...
def _validate_launcher_binding_snapshot(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_3") is not None:
        active_authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_3")
        _require(
            active_authority
            == {
                "schema_version": 1,
...
def _validate_launch_routing_snapshot(
    ...
    *,
    ...
    active = globals().get("SEALED_LAUNCH_ROUTING_V1_3_3")
    if active is not None:
        _require(
...
def _run_distributed_matrix(
    *,
    ...
    if _prepared is None:
        canonical = _canonical_evaluator(evaluator_script)
        activation_lease: admission.QualityStartActivationLeaseV1_3_3 | None = None
        gpu_lease = (
            None
            if coordinator_only
        ...
        try:
            ...
            _require(
                EVALUATOR_IMPLEMENTATION_PATH in contract.V1_3_3_IMPLEMENTATION_PATHS
                and GPU_LOCK_IMPLEMENTATION_PATH in contract.V1_3_3_IMPLEMENTATION_PATHS,
...
def run_matrix(
    *,
    manifest_path: Path = contract.V1_3_3_MANIFEST_PATH,
    ...
    _require(
        ...
        "Controller v1.3.3 is frozen to worker-count 1, worker-index 0, and no coordinator-only mode.",
    )
    ...
    canonical = _canonical_evaluator(evaluator_script)
    gpu_lease = acquire_gpu_lock("p2-direct-controller-exact-fill-v1-3-3-matrix-single", path=gpu_lock_path)
    with ExitStack() as execution_stack:
        ...
        device_guard_lease = _acquire_selected_device_guard(
            label="p2-direct-controller-exact-fill-v1-3-3-matrix-single",
            device_context=device_context,
            scheduler_lease=gpu_lease,
        )
...
def main() -> int:
    ...
    parser.add_argument(
        ...
        help="Frozen canonical v1.3.3 scheduler lock path; custom paths are rejected.",
    )
    parser.add_argument(
        "--worker-index",
        type=int,
        default=0,
        help="Frozen v1.3.3 worker index; must remain 0.",
    )
    parser.add_argument(
        "--worker-count",
        type=int,
        default=1,
        help=(
            "Frozen v1.3.3 worker count; must remain 1."
        ),
    )
    parser.add_argument(
        "--coordinator-only",
        action="store_true",
        help="Unsupported by the frozen v1.3.3 single-worker topology.",
    )
