...
globals().setdefault("SEALED_PYTHON_RUNTIME_V1_3_1", None)
globals().setdefault("SEALED_LAUNCH_AUTHORITY_V1_3_1", None)
globals().setdefault("SEALED_SOURCE_PROVENANCE_V1_3_1", None)
globals().setdefault("SEALED_LAUNCH_ROUTING_V1_3_1", None)

DEFAULT_GPU_LOCK_PATH = contract.DIRECT_GPU_SCHEDULER_LOCK_PATH

EXPERIMENT_ID = contract.V1_3_1_MATRIX_EXPERIMENT_ID
WORKER_EXPERIMENT_ID = contract.V1_3_1_WORKER_LEDGER_EXPERIMENT_ID
ARTIFACT_TYPE = "direct-controller-exact-fill-quality-matrix"
SCHEMA_VERSION = 1
MATRIX_ATTESTATION_PURPOSE = contract.V1_3_1_MATRIX_ATTESTATION_PURPOSE
WORKER_ATTESTATION_PURPOSE = contract.V1_3_1_WORKER_LEDGER_ATTESTATION_PURPOSE
...
EVALUATOR_FD_ENV = "ADAPTIVE_V4_CANONICAL_DIRECT_EXACT_FILL_V1_3_1_EVALUATOR_FD"
IMPORT_INVENTORY_FD_ENV = "ADAPTIVE_V4_DIRECT_EXACT_FILL_V1_3_1_IMPORT_INVENTORY_FD"
...
CELL_CLAIM_NAME = ".p2-direct-controller-exact-fill-v1-3-1-cell.claim"
...
def _validated_source_provenance() -> dict[str, Any]:
    value = globals().get("SEALED_SOURCE_PROVENANCE_V1_3_1")
    _require(
...
def _require_launcher_authority() -> dict[str, Any]:
    authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_1")
    _require(
        authority
        == {
            "schema_version": 1,
...
def _validated_launch_routing(*, expected_selector: str) -> dict[str, Any]:
    value = globals().get("SEALED_LAUNCH_ROUTING_V1_3_1")
    expected_paths = {
        "matrix": (
...
def _validated_launcher_python_runtime() -> dict[str, Any]:
    runtime = globals().get("SEALED_PYTHON_RUNTIME_V1_3_1")
    _require(
...
def _sealed_import_inventory_fd(context: admission.QualityContext) -> int:
    ...
    descriptor = cast(Callable[[str, int], int], create)(
        "adaptive-v4-direct-exact-fill-v1-3-1-import-inventory",
        cast(int, getattr(os, "MFD_CLOEXEC", 0)) | cast(int, allow_sealing),
    )
    try:
        offset = 0
...
def _isolated_evaluator_pycache_prefix(launch_nonce: str) -> Path:
    ...
    path = root / f"adaptive-v4-direct-exact-fill-v1-3-1-pycache-{launch_nonce}"
    _require(
...
def _validate_launcher_binding_snapshot(value: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_1") is not None:
        active_authority = globals().get("SEALED_LAUNCH_AUTHORITY_V1_3_1")
        _require(
            active_authority
            == {
                "schema_version": 1,
...
def _validate_launch_routing_snapshot(
    ...
    *,
    ...
    active = globals().get("SEALED_LAUNCH_ROUTING_V1_3_1")
    if active is not None:
        _require(
    ...
    *,
    ...
    if _prepared is None:
        ...
        try:
            ...
            _require(
                EVALUATOR_IMPLEMENTATION_PATH in contract.V1_3_1_IMPLEMENTATION_PATHS
                and GPU_LOCK_IMPLEMENTATION_PATH in contract.V1_3_1_IMPLEMENTATION_PATHS,
...
def run_matrix(
    *,
    ...
    canonical = _canonical_evaluator(evaluator_script)
    gpu_lease = acquire_gpu_lock("p2-direct-controller-exact-fill-v1-3-1-matrix-single", path=gpu_lock_path)
    with ExitStack() as execution_stack:
        ...
        device_guard_lease = _acquire_selected_device_guard(
            label="p2-direct-controller-exact-fill-v1-3-1-matrix-single",
            device_context=device_context,
            scheduler_lease=gpu_lease,
        )
