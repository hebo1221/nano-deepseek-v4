...
def summarize(*, manifest_path: Path, longsafety_path: Path, ifeval_path: Path) -> dict[str, Any]:
    ...
    for arm in ARMS:
        ...
        _require(
            ...
            and row.get("generated", 0) + sum(row.get("failures_by_type", {}).values())
            == expected_long,
    ...
    for arm in ARMS:
        ...
        _require(
            metrics.get("expected_prompts") == expected_ifeval
            and metrics.get("scored_prompts", 0) + metrics.get("scorer_failures", 0)
            == expected_ifeval
            and all(
    ...
    _require(
        paired.get("paired_prompts") == expected_ifeval
        and paired.get("bootstrap_replicates")
