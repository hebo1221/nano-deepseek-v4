...
def load_rows(
    ...
    for needle_count in NEEDLE_COUNTS:
        ...
        expected_per_needle = (
            contract["primary_bins_through_128k"]
            * contract["samples_per_bin_per_needle_count"]
        )
...
def main() -> None:
    ...
    try:
        ...
        for arm in pending:
            ...
            _require(
                all(existing[index]["example_id"] == rows[index]["example_id"] for index in range(len(existing))),
