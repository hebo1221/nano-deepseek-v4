...
SUMMARY_EXPERIMENT_ID = "p3-natural-adaptive-quota-mrcr-audit-v1"
RUNNER = Path("research/adaptive_v4_memory/scripts/run_p3_mrcr.py")
...
def _dependency(metadata: Any, label: str) -> dict[str, str]:
    _require(isinstance(metadata, dict), f"Missing adaptive MRCR {label} dependency.")
    path = Path(metadata.get("path", ""))
    _require(
        path.is_file() and metadata.get("sha256") == sha256(path),
        f"Adaptive MRCR {label} dependency drifted.",
    )
...
def _verify_adaptive_cell(
    ...
    *,
    ...
    _require(
        ...
        f"Adaptive MRCR cell identity drifted for {arm}.",
    )
    ...
    _require(
        ...
        and set(prerequisites) == {"adaptive_ruler", "baseline_mrcr"},
        f"Adaptive MRCR prerequisite set drifted for {arm}.",
    )
    assert isinstance(prerequisites, dict)
    contracts = {
        "adaptive_ruler": ("p3-natural-adaptive-quota-ruler-audit-v1", 65_000),
        "baseline_mrcr": ("p3-natural-mrcr-audit-v1", 3_000),
    }
    ...
    for name, (experiment_id, predictions) in contracts.items():
        ...
        _require(verified == metadata, f"Adaptive MRCR prerequisite drifted: {name}.")
    ...
    _require(
        ...
        f"Adaptive MRCR prerequisite identity drifted for {arm}.",
    )
    ...
    _require(
        ...
        f"Adaptive MRCR sequence dependency set drifted for {arm}.",
    )
    verified_sequence = {
    ...
    _require(
        ...
        f"Adaptive MRCR sequence identity drifted for {arm}.",
    )
    return {
    ...
    for row in records:
        ...
        if row["status"] == "failure":
            _require(
                ...
                f"Unregistered adaptive MRCR failure: {row['example_id']}/{arm}.",
            )
        else:
            _require(
                failure is None,
                f"Scored adaptive MRCR example carries a failure: {row['example_id']}.",
            )
        token_digest = row.get("input_token_ids_sha256")
        _require(
            isinstance(token_digest, str) and len(token_digest) == 64,
            f"Missing exact MRCR token digest: {row['example_id']}.",
        )
        audit = row.get("quota_physical_audit")
        if audit is None:
            _require(
                row["status"] == "failure" and row["hot_resident_bytes"] == 0,
                f"Successful MRCR context prefill lacks quota audit: {row['example_id']}/{arm}.",
            )
            ...
            _require(
                ...
                f"Audited MRCR context prefill has zero hot bytes: {row['example_id']}/{arm}.",
            )
...
def analyze_pairs(
    ...
    *,
    ...
    token_bins: int = 5,
    examples_per_cell: int = 100,
) -> dict[str, Any]:
    _require(
        len(fixed) == len(adaptive) == expected_examples,
        "Adaptive MRCR paired example count drifted.",
    )
    ...
    _require(
        ...
        "Adaptive MRCR paired identities drifted.",
    )
    ...
    cells: dict[tuple[int, int], list[float]] = {
        ...
        for token_bin in range(token_bins)
    }
    ...
    for identifier in sorted(fixed_by_id):
        ...
        _require(
            ...
            f"Adaptive MRCR paired input drifted: {identifier}.",
        )
        needle = int(reference["needle_count"])
        token_bin = int(reference["official_bin_index"])
        _require((needle, token_bin) in cells, f"Unknown adaptive MRCR cell: {identifier}.")
        ...
        cells[(needle, token_bin)].append(difference)
        ...
        if reference_quota is not None and treatment_quota is not None:
            _require(
                ...
                f"Adaptive MRCR global quota differs: {identifier}.",
            )
    ...
    _require(
        all(len(values) == examples_per_cell for values in cells.values()),
        "Adaptive MRCR needle/token-bin grid is incomplete.",
    )
    ...
    for index, ((needle, token_bin), values) in enumerate(cells.items(), start=1):
        estimate = paired_bootstrap(values, seed=seed + index, resamples=resamples)
        key = f"{needle}n/bin-{token_bin}"
        raw_p[key] = float(estimate["two_sided_bootstrap_p"])
        cell_rows.append(
            {"needle_count": needle, "official_bin_index": token_bin, **estimate}
        )
    _require(
        len(raw_p) == statistics["holm_family_size"],
        "Adaptive MRCR Holm family size drifted.",
    )
    adjusted = holm_bonferroni(raw_p)
    for row in cell_rows:
        key = f"{row['needle_count']}n/bin-{row['official_bin_index']}"
    ...
    checks = {
        ...
        >= gate["overall_score_difference_minimum"],
        "paired_bootstrap_lower_bound": overall["paired_bootstrap_95_ci"][0]
        >= gate["paired_bootstrap_lower_bound_minimum"],
        "nonnegative_needle_bin_count": sum(
            row["mean_difference"] >= 0.0 for row in cell_rows
        )
        >= gate["nonnegative_needle_bin_count_minimum"],
        "worst_needle_bin_regression": min(
            row["mean_difference"] for row in cell_rows
        )
        >= gate["worst_needle_bin_regression_minimum"],
    ...
    return {
        ...
        "overall": overall,
        "by_needle_token_bin": cell_rows,
        "multiplicity": {
            "method": "Holm-Bonferroni",
            "family_size": len(raw_p),
            "family": "all fifteen frozen MRCR needle-count by token-bin effects",
        },
...
def summarize(
    *, adaptive_manifest_path: Path, natural_manifest_path: Path, result_root: Path
    ...
    _require(all(path.is_file() for path in cell_paths.values()), "Missing adaptive MRCR cells.")
    ...
    _require(len(identifiers) == EXPECTED_EXAMPLES, "Adaptive MRCR identifier count drifted.")
    revisions = expected_record_revisions("MRCR", natural_manifest)
    ...
    _require(
        ...
        "Adaptive MRCR arms used different frozen prerequisites.",
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Audit adaptive-quota MRCR results.")
    parser.add_argument(
        "--adaptive-manifest",
        type=Path,
        default=Path(
    ...
    _require(not dirty, "Adaptive MRCR summarization requires a clean source tree.")
    payload = summarize(
