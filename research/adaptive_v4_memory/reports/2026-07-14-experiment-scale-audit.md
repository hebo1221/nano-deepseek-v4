...
predictions, and two 108-cell systems matrices with 30 measured repetitions per
cell. These units are reported separately and are never combined into a
...
| [The Pitfalls of KV Cache Compression](https://arxiv.org/abs/2510.00231) | Multi-instruction degradation and system-prompt leakage across eviction choices | Our four-family safety suite directly tests the identified failure modes, but remains synthetic retention evidence. |
...
4. Keep reference PyTorch, checked static continuous batching, and unavailable
   external production runtime as separate evidence tiers.
5. After primary completion, add a second instruction-tuned model family only
